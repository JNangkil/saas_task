...
export class LineChartComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['data'] || changes['datasets'] || changes['labels'] || changes['title'] || changes['yAxisLabel'] || changes['xAxisLabel']) {
  ...
  private updateChartData(): void {
    ...
    this.chartData = {
      ...
      datasets: this.datasets.map((dataset, i) => ({
        ...
        backgroundColor: (dataset.color || colors[i % colors.length]) + '20',
        borderWidth: 2,
        fill: true,
        tension: 0.4,
        pointRadius: 4,
