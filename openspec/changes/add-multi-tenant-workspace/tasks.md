...
- [ ] 1.1 Create `tenants` table migration (id, name, slug, logo_url, billing_email, settings JSON, status, locale, timezone, created_at, updated_at)
- [ ] 1.2 Create `workspaces` table migration (id, tenant_id, name, description, color, icon, is_archived, created_at, updated_at, deleted_at)
- [ ] 1.3 Create `tenant_user` pivot table migration (tenant_id, user_id, role, invited_at, joined_at)
- [ ] 1.4 Create `workspace_user` pivot table migration (workspace_id, user_id, role, joined_at)
- [ ] 1.5 Add database indexes for performance (tenant_id, workspace_id, slug, status)
- [ ] 1.6 Create foreign key constraints with appropriate cascade rules

## 2. Laravel Models

- [ ] 2.1 Create Tenant model with relationships (workspaces, users, boards)
- [ ] 2.2 Create Workspace model with relationships (tenant, users, boards) and soft deletes
- [ ] 2.3 Update User model with tenant and workspace relationships
- [ ] 2.4 Define model factories for Tenant and Workspace

## 3. Multi-Tenant Middleware & Scoping

- [ ] 3.1 Create TenantResolution middleware (resolve from subdomain → header → JWT claim)
...
- [ ] 5.1 Create TenantController with index, show, store, update methods
- [ ] 5.2 Implement tenant archive/deactivate endpoint
...
- [ ] 6.1 Create WorkspaceController with index, show, store, update methods
- [ ] 6.2 Implement workspace archive/delete endpoint (soft delete)
...
- [ ] 7.1 Create TenantService with API integration methods
- [ ] 7.2 Create WorkspaceService with API integration methods
- [ ] 7.3 Implement workspace state management using BehaviorSubject
- [ ] 7.4 Create workspace context interface and types
- [ ] 7.5 Add HTTP interceptor to include tenant context in requests

## 8. Angular UI - Workspace Selector

- [ ] 8.1 Create WorkspaceSelectorComponent (dropdown in navbar)
- [ ] 8.2 Implement workspace switching functionality
- [ ] 8.3 Persist last selected workspace (localStorage or backend)
- [ ] 8.4 Handle workspace loading states and errors
- [ ] 8.5 Style selector with consistent design system

## 9. Angular UI - Workspace Management

- [ ] 9.1 Create WorkspaceListComponent (list all workspaces for user)
- [ ] 9.2 Create WorkspaceFormComponent (create/edit workspace)
...
- [ ] 10.1 Handle 403/404 responses for tenant/workspace access errors
- [ ] 10.2 Add toast notifications for workspace operations
- [ ] 10.3 Implement navigation guards for workspace context
