...
export class BoardSettings implements OnInit {
  ...
  ngOnInit(): void {
    combineLatest([
      ...
      this.tenantId = context.currentTenant?.id || context.currentWorkspace?.tenant_id || '';

      if (this.workspaceId && this.boardId && this.tenantId) {
        this.loadBoard();
      }
    });
  }
  ...
  saveChanges(): void {
    ...
    this.boardService.updateBoard(this.tenantId, this.workspaceId, this.boardId, updates).subscribe({
      next: (updated) => {
        this.board = updated;
        this.isSaving = false;
        this.workspaceShellService.updateBoard(updated);
      },
      error: (err) => {
  ...
  archiveBoard(): void {
    ...
    this.boardService.archiveBoard(this.tenantId, this.workspaceId, this.boardId).subscribe(() => {
      this.workspaceShellService.removeBoard(Number(this.boardId));
      this.router.navigate(['/app', this.workspaceId, 'home']);
    });
  }
