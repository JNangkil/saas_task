...
export class DynamicTaskTableComponent implements OnInit, OnDestroy {
    ...
    private loadSavedFilters(): void {
        ...
        this.workspaceContextService.context$.pipe(
            ...
            this.boardViewPreferenceService.getPreferences(
                ...
                this.statusFilter$.next(filters.status ?? []);
                this.priorityFilter$.next(filters.priority ?? []);
                this.assigneeFilter$.next(filters.assignee_id ?? []);
            });
        });
    }
            ...
            this.boardViewPreferenceService.updatePreferences(
                ...
                this.boardId!.toString(),
                {
                    filters: {
                        ...
                        priority: this.priorityFilter$.value.length ? this.priorityFilter$.value : undefined,
                        assignee_id: this.assigneeFilter$.value.length ? this.assigneeFilter$.value : undefined
                    }
                }
            ).subscribe();
        });
